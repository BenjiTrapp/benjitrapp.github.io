...
def capture_traffic(pid, interface='eth0', duration=10, output_file='output.pcap'):
    import pyshark  # lazy import — only needed for traffic capture (requires Wireshark)
    process = psutil.Process(pid)
    connections = process.connections()
...
def monitor_process_details(pid):
    try:
        while True:
            ...
            time.sleep(0.5)  # Sleep for 0.5 seconds before refreshing
            print("\n" + "="*30 + "\n")
    except KeyboardInterrupt:
        print("\nMonitoring stopped.")
...
defender_tool_processes = {
    ...
    "Microsoft Smart Screen": ["smartscreen.exe"],
    "Carbon Black (Cloud)": ["cb.exe", "RepMgr", "RepUtils", "RepUx", "RepWAV", "RepWSC"],
    "BitDefender": ["bdagent.exe", "bdredline.exe", "bdreinit.exe", "bdsubwiz.exe", "bdwtxag"],
